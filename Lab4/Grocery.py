...
while check == False:
    ...
    if option == "1":
        ...
        print(Prices)
        try:
            x = input("what value corresponding to food/price would you like to add to your cart? ")
            x = int(x)
    ...
    if option == "2":
        try:
            x = input("what value corresponding to food/price would you like to remove to your cart? ")
            x = int(x)
            y = input(f"How many {Food[x]} would you like? ")
            y = int(y)
            total = total - y * Prices[x]
